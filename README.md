...
[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/wlxfuture/maotu002)
...
4. 在部署界面上，您需要配置以下内容：
    ...
    - **环境变量**：
        ...
        - `AUTH_PASSWORD`：管理员密码（选择加密/Secret 类型）
        - `AUTH_SECRET`：JWT 密钥（使用随机字符串并选择加密/Secret 类型）
5. 点击"部署"按钮
...
密码和 JWT 密钥不要写入 `wrangler.jsonc`，请通过 Wrangler 交互式命令配置：
